...
## Где живёт сайт (не перепутай)

**`wedesignerz.com` — это НЕ этот проект.** Домен обслуживает отдельный репозиторий
`androsovtema/wdcom` (старый статичный сайт-портфолио, без Supabase и авторизации).
**Клубный сайт из этого репо публикуется на `https://androsovtema.github.io/VibeClub/`** —
домен к нему пока не привязан. Проверять правки нужно там. Детали и последствия —
...
- **Supabase JS вендорен локально** (`js/vendor/supabase-js@2.110.3.mjs`). Не возвращай
  импорт с CDN (`esm.sh`) — это закрытая находка аудита SEC-07. Обновление версии —
  только через PR: сгрузить новый bundle, прогнать `npm run security-check`.
