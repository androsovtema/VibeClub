# 16 — Статус безопасности (актуально на 2026-07-17)
...
3. T-LOC/T-CUTOVER закрыты; production использует self-host. T-CONSENT-02
   ...
   flash-fix живьём принят. Финальный security-check после live migration
   прошёл 29/29 с обычным member JWT; профиль восстановлен, сессия отозвана.
   **T-CONSENT-RECONSENT/T-CONSENT закрыты 2026-07-17**; следующий legal-шаг —
   уведомление РКН.
...
6. Для обычной операторской проверки пользователей использовать закрытый
   Supabase Studio по `17-operator-support.md`; не публиковать Studio в
   Caddy/DNS и не выводить PII в отчёты.
7. Полный дальнейший маршрут до анонса — `18-project-roadmap.md`.
