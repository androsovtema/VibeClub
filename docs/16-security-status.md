...
3. T-LOC/T-CUTOVER закрыты; production использует self-host. T-CONSENT-02
   ...
   375 px зелёные; inventory не делал backfill/удалений. Один сохраняемый
   legacy admin без active processing v4 блокирует legal closure — следующая
   задача **T-CONSENT-RECONSENT**, затем уведомление РКН.
