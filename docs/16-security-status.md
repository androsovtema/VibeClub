...
3. T-LOC/T-CUTOVER закрыты; production использует self-host. T-CONSENT-02
   технически применён 2026-07-16, регистрации после окна включены. До
   уведомления РКН закрыть member JWT security-check, CAPTCHA новой регистрации,
   ручную 375 px проверку и legacy inventory без backfill.
