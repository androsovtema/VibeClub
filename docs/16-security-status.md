...
cloud и self-host. Операционный остаток T-CUTOVER — деплой CSP,
freeze+дельта-сверка без реимпорта и полный e2e до переключения фронта.
...
3. URL Configuration и CAPTCHA закрыты в cloud и self-host; пароль на обеих
   ...
   задеплоить CSP, затем в freeze-окне подтвердить нулевую дельту без
   реимпорта и пройти шаги 7–8.1 `infra/RUNBOOK.md`.
