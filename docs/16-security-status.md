...
запрос без CAPTCHA отбивается. Данные уже перенесены и повторно сверены между
cloud и self-host. CSP задеплоен commit `4a21ce6` и проверен на живом cloud-проде.
Операционный остаток T-CUTOVER — freeze+дельта-сверка без реимпорта и полный
e2e до переключения фронта.
...
- Остаётся пройти полный self-hosted e2e и `security-check` после переноса;
  это не новая дыра, а обязательная проверка cutover.
...
3. URL Configuration и CAPTCHA закрыты в cloud и self-host; пароль на обеих
   сторонах минимум 12, данные и Storage уже перенесены. Следующий gate:
   в freeze-окне подтвердить нулевую дельту без реимпорта и пройти шаги 7–8.1
   `infra/RUNBOOK.md`. CSP уже задеплоен и не сломал cloud-прод.
4. Проверка защиты БД при включённой CAPTCHA: взять `access_token` обычного
   (`member`, не admin) пользователя из браузерной сессии и запустить
   `WDZ_TEST_JWT='<access_token>' npm run security-check`. Токен не сохранять
   в файлы и не вставлять в чат/логи.
